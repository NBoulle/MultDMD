...
% URL: https://uk.mathworks.com/matlabcentral/fileexchange/34428-voronoilimit-varargin
    ...
    for ij=1:length(C)
        if sum(allVixinp(C{ij}))~=length(C{ij})
            ...
            for ii=1:4
                ...
                if ~isempty(xip)
                    intersect_poly(ii)=true;
                end
            end
            if any(intersect_poly) %possibly expand outer points
                poly_ok(ij)=2;
            end
            
            if poly_ok(ij)==1
                poly_ok(ij)=0;
                poly1 = polyshape(crs(:,1),crs(:,2));
                poly2 = polyshape(V(C{ij},1),V(C{ij},2));
                polyout = intersect(poly1,poly2);
                xb = polyout.Vertices(:,1);
                yb = polyout.Vertices(:,2);
                xb = [xb; xb(1)];
                yb = [yb; yb(1)];
                ix=nan(1,length(xb));
                for il=1:length(xb)
...
%                                     nb=nb(poly_ok(nb)==0);
                                    if ~isempty(nb)
                                        ...
                                        for in=1:length(nb)
                                            poly1 = polyshape(V(C{nb(in)},1),V(C{nb(in)},2));
                                            poly2 = polyshape(poly(:,1),poly(:,2));
                                            polyout = intersect(poly1,poly2);
                                            xb = polyout.Vertices(:,1);
                                            xb = [xb; xb(1)];
                                            if ~isempty(xb)
...
%                 if im==2 && ~isempty(ixok)
    ...
    if exist('bs_int','var')
        ...
        for ii=1:length(bs_int)
            ...
            ctr=1;
            for ij=1:length(C)
                poly1 = polyshape(V(C{ij},1),V(C{ij},2));
                poly2 = polyshape(bs_int{ii}(:,1),bs_int{ii}(:,2));
                polyout = subtract(poly1,poly2);
                pbx = polyout.Vertices(:,1);
                pby = polyout.Vertices(:,2);
                pbx = [xb; xb(1)];
                pby = [yb; yb(1)];
                if ~isempty(pbx)
